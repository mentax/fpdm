...
class FPDM {
	...
	public function __construct() {
		...
		switch ($num_args) {
			...
			case FPDM_STATIC:
				if ($args[0] == '[_STATIC_]') {
					break;
				} //static use, caller is anonymous function defined in _set_field_value
	...
	**/
	public function load_file($type, $content = null) {
		//------------------------------------
		switch ($type) {
			case "PDF" :
				if ($content) {
			...
			case "FDF" :
				if (!is_null($content)) {
					...
					} elseif (is_string($content)) { //String
	...
	**/
	public function Merge($flatten = false, $read_only = false) {
		//------------------------------
	...
	**/
	public function get_buffer($pdf_file = '') {
		//---------------------
		if ($pdf_file == '') {
			$buffer = implode("\n", $this->pdf_entries);
		} else {
	...
	**/
	public function replace_value($matches, $value) {
		//----------------------------------------------
	...
	**/
	public function _set_field_value($line, $value) {
		//----------------------------------------
	...
	public function _encode_value($str) {
	...
	public function _set_field_value2($line, $value, $append, $read_only) {
		$CurLine = $this->pdf_entries[$line];
		$OldLen = strlen($CurLine);
	...
	*@param string $value the new value to set
	**/
	public function set_field_value($type, $name, $value, $read_only = false) {
		//------------------------------------
		$verbose_set = ($this->verbose && ($this->verbose_level > 1));
	...
	**/
	public function set_field_tooltip($name, $value) {
		//------------------------------------
		$offset_shift = 0;
		$verbose_set = ($this->verbose && ($this->verbose_level > 1));
	...
	**/
	public function _hex2bin($hexString) {
	...
	**/
	 ...
	 */
	public function get_xref_table() {
		//------------------------
	 ...
	 */
	public function get_xref_start() {
		//------------------------
	 ...
	 */
	public function get_xref_start_line() {
		//-------------------------------
	 ...
	 */
	public function get_xref_start_value() {
		//-------------------------------
	 ...
	 * @return int the wished xrefstart offset value
	 */
	public function read_xref_start_value() {
		//------------------------------
	 ...
	 */
	public function get_offset_object_value($object_id) {
		//--------------------------------------------
	 ...
	 * @return int the wished offset
	 */
	public function read_offset_object_value($object_id) {
		//------------------------------
		$buffer = $this->get_buffer();
	 ...
	 *
	 */
	public function fix_xref_start() {
		//-------------------------
	 ...
	 */
	public function _get_offsets_starting_from_zero() {
		//-------------------------------------------
	 ...
	 */
	public function _get_positions_ordered() {
		//--------------------------------
	 ...
	 *
	 */
	public function fix_xref_table() {
		//------------------------
	 ...
	 */
	public function _apply_offset_shift($from, $shift) {
		//------------------------------------------
		$offsets = &$this->shifts;
		$params = [$from,$shift];
	...
	**/
	 ...
	 * @param string $value the new text value
	 */
	public function _set_text_value($stream, $value) {
		//---------------------------------------
	...
	public function _extract_pdf_definition_value($name, $line, &$match) {
		//-----------------------------------------------------------
		global $FPDM_REGEXPS;
	...
	public function extract_pdf_definition_value($name, $line, &$match) {
		//-----------------------------------------------------------
		global $FPDM_REGEXPS;
						...
						if (preg_match("/endobj/", $CurLine, $match)) {
							...
							if (array_key_exists("length", $stream)) { //length is mandatory
								...
								*/
								...
								} elseif (preg_match("/^\/MaxLen\s+(\d+)/", $CurLine, $values)) {
									$maxLen = $values[1];
				...
				switch ($xref_table) {
					...
					default:
						...
						if ($refs_count > 0) {
							...
							if ($trailer_table == 1) { //should be <<
							...
							} elseif (($trailer_table > 0) && ((!is_null($id_def)) || preg_match("/^\/(Size|Root|Info|ID|DocChecksum)/", $CurLine, $match))) {
								...
								if (preg_match("/>>/", $CurLine, $match)) {
									$trailer_table = -1;
								}//negative value: expects startxref to follow
							} else {
	...
	*/
	public function _protectContentValues($content) {
		//-------------------------------------------------
	...
	*/
	public function _unprotectContentValues($content) {
		//--------------------------------------------------
		$content = str_replace("$@#", "\\(", $content);
		$content = str_replace("#@$", "\\)", $content);
